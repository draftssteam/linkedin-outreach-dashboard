...
/**
 * Map a raw CloselyHQ webhook payload to our internal DB row.
 * CloselyHQ sends varying shapes depending on event; we normalise here.
 */
function parsePayload(raw) {
  // CloselyHQ can nest data under different keys; handle gracefully
  const event_type = raw.event || raw.type || raw.event_type || 'unknown';

  // Profile identity (Closely may send account email or id)
...
app.post('/webhook/closely', (req, res) => {
  ...
  try {
    ...
    // Fetch full row to broadcast
    const saved = db.getEventById(id);
    broadcastSSE('new_event', saved);

  } catch (err) {
    logWebhook(req.body, `ERROR: ${err.message}`);
    // Do NOT throw — we already responded 200
  }
});

// ─── TEST WEBHOOK (simulate without CloselyHQ) ────────────────────────────────
...
app.get('/api/live-feed', (req, res) => {
  ...
  // Send keep-alive comment every 20s to prevent proxy timeouts
...
app.get('/api/events', (req, res) => {
  try {
    const result = db.getEvents(req.query);
    res.json(result);
  } catch (err) {
